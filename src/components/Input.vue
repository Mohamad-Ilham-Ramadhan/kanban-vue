...
<template>
   <div class="relative w-full">
      <input :name="name" :id="name" :type="type" :value="inputValue" :placeholder="placeholder" @input="handleChange" :disabled="props.disabled"
         @blur="handleBlur" :class="['transition-colors bg-transparent rounded border-2 text-sm dark:text-white text-black block px-6 py-2 w-full outline-none', errorMessage ? 'border-red-500' : 'border-slate-400 dark:border-gray-600 focus:border-primary dark:focus:border-primary', props.disabled ? 'opacity-30' : '',]"/>
